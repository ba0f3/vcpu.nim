...
proc writeArg(a: Assembler, t: Token, prev = Token(kind: NONE)) =
  ...
  of NAME:
    ...
    if prev.kind == REG:
      ...
      size = sizeof(int)
      if not a.labels.hasKey(t.n):
        a.labels[t.n] = getFuncAddr(t.n)
...
proc compileString*(a: Assembler, input: string, file = ""): TaintedString =
  if file.len != 0:
    a.basePath = expandFilename(file).splitPath.head
  a.tokenizer(input)
  ...
  when not defined(release):
    hexdump(result.cstring, result.len)
